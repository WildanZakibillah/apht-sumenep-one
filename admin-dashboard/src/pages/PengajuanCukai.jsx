...
const PengajuanCukai = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    const { data, error } = await supabase.from('cukai_requests').select('*, factories(name, code)').order('created_at', { ascending: false });
    ...
    <div className="space-y-5 max-w-[1400px] mx-auto">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
        <div className="border-b border-gray-100 dark:border-gray-800 px-5 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          ...
          <SearchBar value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari pabrik, dokumen..." className="md:w-96" />
        </div>
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
              {filteredRequests.length === 0 ? (
                ...
                <tr key={req.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-sm font-semibold text-gray-900 dark:text-white">{req.factories?.name || '-'}</td>
