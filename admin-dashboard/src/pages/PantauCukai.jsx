...
const PantauCukai = () => {
  ...
  const getPercentage = (alloc) => {
    ...
    return Math.round(((alloc.quota - (alloc.used || 0) - (alloc.damaged || 0)) / alloc.quota) * 100);
  };
  ...
  const chartData = allocations.map((a) => ({
    ...
    used: a.used || 0,
    remaining: (a.quota || 0) - (a.used || 0) - (a.damaged || 0),
    damaged: a.damaged || 0,
  }));
  ...
  const openEdit = (a) => { setEditing(a); setForm({ factory_id: a.factory_id || '', quota: a.quota || 0, used: a.used || 0, damaged: a.damaged || 0, period: a.period || currentPeriod() }); setShowModal(true); };
  const handleSubmit = async (e) => {
    e?.preventDefault?.();
    if (!form.factory_id || !form.period.trim()) { toast.warning('Pabrik dan periode wajib diisi'); return; }
    setSaving(true);
    try {
      const payload = { factory_id: form.factory_id, quota: parseInt(form.quota) || 0, used: parseInt(form.used) || 0, damaged: parseInt(form.damaged) || 0, period: form.period.trim() };
        ...
        <div className="lg:col-span-2 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={chartData} margin={{ top: 5, right: 5, left: -10, bottom: 0 }}>
              ...
              <XAxis dataKey="code" tick={{ fontSize: 10, fill: chartColors.text }} axisLine={false} tickLine={false} />
            ...
            {allocations.filter((a) => getPercentage(a) <= 10).length === 0 ? (
              ...
              allocations.filter((a) => getPercentage(a) <= 10).map((alloc) => (
                <div key={alloc.id} className="p-3.5 rounded-lg bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30">
                  ...
                  <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-1">Sisa: {formatNumber(alloc.quota - alloc.used - (alloc.damaged || 0))} dari {formatNumber(alloc.quota)}</p>
                </div>
              ))
            )}
          </div>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
              {filteredAllocations.length === 0 ? (
                <tr><td colSpan="8" className="px-5 py-10 text-center text-gray-400 dark:text-gray-500 text-sm">{search ? 'Tidak ditemukan' : 'Belum ada alokasi'}</td></tr>
              ) : filteredAllocations.map((alloc) => {
                const percent = getPercentage(alloc);
                const remaining = (alloc.quota || 0) - (alloc.used || 0) - (alloc.damaged || 0);
                return (
